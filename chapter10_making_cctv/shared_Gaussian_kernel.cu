...
void shared_CUDA_Gaussian_Filter(uchar *pcuSrc, uchar *pcuDst,
	int w, int h, float *cuGkernel, int kernel_size) {
	// 16 x 16 쓰레드의 블록과 그 grid 사이즈 저장
	dim3 grid = dim3(w / 16, h / 16, 1);
	dim3 block = dim3(16, 16, 3);
	...
	// 각 pixel 별 CUDA Gaussain Filter 진행
	cuda_shared_Filter_2D << < grid, block, sizeof(float) * kernel_size * kernel_size >> >
		(pcuSrc, pcuDst, w, h, cuGkernel, kernel_size, kernel_size);
